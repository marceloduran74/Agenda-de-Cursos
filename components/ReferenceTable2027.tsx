...
interface ReferenceTable2027Props {
  courses: CourseDefinition[];
  isPrivacyMode: boolean;
  onUpdateValue: (courseName: string, newValue: number) => void;
}

export const ReferenceTable2027: React.FC<ReferenceTable2027Props> = ({ courses, isPrivacyMode, onUpdateValue }) => {
  return (
    <div className="bg-slate-800 rounded-xl shadow-lg shadow-black/20 overflow-hidden">
      <div className="p-4 bg-slate-700/50 border-b border-slate-700">
        <h2 className="text-xl font-bold text-slate-100">Valor 2027 - Tabela de Referência</h2>
      </div>
            ...
            {courses.sort((a, b) => a.name.localeCompare(b.name)).map(course => {
              ...
              return (
                <tr key={course.name} className="bg-slate-800 border-b border-slate-700 hover:bg-slate-700/50">
                  ...
                  {isPrivacyMode ? (
                    <td className="px-6 py-4 text-right">R$ ••••••</td>
                  ) : (
                    <td className="p-0"> {/* Remove padding from the cell to allow input to fill it */}
                      <input
                        ...
                        className="w-full text-right bg-transparent border-0 rounded-none py-4 px-6 focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-slate-700"
